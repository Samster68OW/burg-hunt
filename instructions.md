...
The script `control-center.js` controls which adventures are currently active and advertised.
- `huntList`: This array lists the objects that contain each adventure's data. Adding a new object will add the adventure to the dropdowns and track it during play.
- `activeHunt`: Defaults to false. When a adventure is being advertised, this variable needs to be changed to the index of the adventure in `huntList`.
- `activeHuntStatus`: Defaults to false. When `activeHunt` is a number, this variable becomes relevant.
    - `false`: This means that the selected adventure is coming soon. It cannot be played, but it will be showcased on the Home page and the dropdown.
    - `true`: This means that the selected adventure is active and available to be played. It will be showcased on the Home page and the dropdown.
- `latestVersion`: Displays the latest version of Club Penguin Zero that the website is built for. Controls nothing.

## Adding Adventures

### Creating the Adventure's Data

1. Navigate to `data/hunt-data.js`.
2. Copy one of the large objects and rename it.
3. Fill in the elements of the object.
    - `title`: The name of the scavenger adventure.
    - `imgSource`: Create a folder in `images/item` to hold all of the images for the adventure. Place the name of the folder here.
        - Be sure to add an image called `postcard.png` to this folder.
        - If you are using the linear adventure style, you can add images of each object to this folder.
    - `availability`: Displays below the button on the adventure's main page. Effects nothing.
    - `postcardEarnedID`: See the below section.
    - `style`: This can be either of the following:
        - `"linear"`: A linear adventure only shows one object and its image at a time.
        - `"free"`: A free adventure shows every object at a time with no images.
        ...
        - `"mascot-visit"`: A scripted and customized experience.
...
4. Once you are ready to add your scavenger adventure to the site, find the array `huntList` at the top of `control-center.js` and add your adventure variable to it.
5. Remember to save!

###  Adding the Adventure's Postcard
